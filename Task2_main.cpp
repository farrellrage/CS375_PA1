...
int main(int argc, char **argv)
{
	...
	//Integer to hold the length of the longest common subsequence between
	...
	if (argc != 4)
	{
		...
		cout << "Error - Invalid number of arguments" << endl
			...
			<< " <filex.txt> <filey.txt> <output2.txt>" << endl;
	...
	fout << "Longest Common Subsequence: Recursive, No Memoization" << endl;

	//Get the system time before finding the LCS
	timeStart = chrono::system_clock::now();

	LCSLength = LCSRecursiveNoMemoization(X, Y, lenX, lenY);
	...
	//Output the LCS length and algorithm running time
	fout << "LCS length: " << LCSLength << " characters" << endl;
